...
export function SeiriPhase({ itens }: Props): JSX.Element {
  ...
  const ZONAS = [
    { id: 'manter', titulo: t(lang, 'seiri.zona.manter.t'), descricao: t(lang, 'seiri.zona.manter.d'), cor: '#3FA34D' },
    { id: 'red_tag', titulo: t(lang, 'seiri.zona.redtag.t'), descricao: t(lang, 'seiri.zona.redtag.d'), cor: '#C9A227' },
    { id: 'descartar', titulo: t(lang, 'seiri.zona.descartar.t'), descricao: t(lang, 'seiri.zona.descartar.d'), cor: '#E4572E' },
  ]
    ...
    <div className="space-y-4">
      <div className="rounded-2xl bg-white/10 p-4">
        <p className="mb-3 text-sm font-semibold text-white/80">
          {t(lang, 'seiri.bench', {
            n: pendentes.length,
            w: pendentes.length === 1 ? t(lang, 'seiri.item') : t(lang, 'seiri.items'),
          })}
        </p>
        <div className="flex min-h-[88px] flex-wrap gap-3">
          <AnimatePresence>
            {pendentes.map((item) => (
              <motion.div key={item.id} layout exit={{ scale: 0, opacity: 0 }}>
                <Draggable
                  ariaLabel={t(lang, 'seiri.itemAria', { nome: item.nome, dica: item.dica })}
          ...
          {pendentes.length === 0 && (
            <p className="self-center text-white/70">{t(lang, 'seiri.done')}</p>
          )}
        </div>
      </div>
